...
public class PixelPerfectCollisionChecker {
	...
	public static final int NO_COLLISION_FOUND = -1;
	public static boolean checkCollision(final IShape pA, final IPixelPerfectMask pMaskA, final IShape pB, final IPixelPerfectMask pMaskB) {
		...
		aLocToBLoc.transformNormal(X_StepVersor);
		aLocToBLoc.transformNormal(Y_StepVersor);

		//find the pixel in B corresponding to the top-left vertex of the collision bounding box
		float[] initialPixelLocB = new float[]{minX, minY};
		aLocToBLoc.transform(initialPixelLocB);
	 ...
	 * @param pMaskA
	 * @param pVertices
	 * @return The index of the first vertex found colliding with the RectangularShape. NO_COLLISION_FOUND otherwise
	 */
	public static int checkCollsion(final IShape pA, final IPixelPerfectMask pMaskA, final float[] pVertices) {
		...
		for(int i = 0; i < pVertices.length; i += 2) {
			currentVertex[0] = pVertices[i];
			currentVertex[1] = pVertices[i +1];
			sceneToLoc.transform(currentVertex);
